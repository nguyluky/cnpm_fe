        ...
        const fetchRoutes = async () => {
            try {
                ...
                for (const route of data) {
                    const schedule = await api.getAllSchedules({
                        routeId: route.id,
                    })
                    const scheduleData = schedule?.data?.data?.data || [];
                    // append to schedules if not exist
                    for (const sch of scheduleData) {
                        if (!schedules.find((s) => s.id === sch.id)) {
    ...
    return (
        <>
            <div className="h-screen flex flex-col">
                <div
                    className={`flex-1 ${isFullScreen ? "hidden" : "block"
                        } transition-all`}
                >
                ...
                <div
                    className={`flex-1 rounded-t-2xl bg-white border-t border-gray-300 ${isFullScreen ? "h-full" : "block"
                        } transition-all`}
                >
                    ...
                    {/* List of routes */}
                    <div
                        className={`mt-4 px-4 ${isFullScreen ? "h-full" : "h-[40vh]"
                            } overflow-y-auto`}
                    >
                        {schedules.length === 0 ? (
                            ...
                            schedules.map((schedule, idx) => (
                                ...
                                >
                                    <div
                                        ...
                                        onClick={(e) => {
                                            e.preventDefault();
                                            handleConfirmation(schedule.route.id);
                                        }}
                                    >
                                        ...
                                        <p className="text-sm text-gray-500 truncate whitespace-nowrap">
                                            Thời gian hoạt động:{" "}
                                            {
                                                (() => {
                                                    ...
                                                    // format dayOfWeek
                                                    // e.g. [1,2,3,4,5] => "Thứ 2 - Thứ 6"
                                                    const days = schedule.times.dayOfWeek
                                                        .map((day) => {
                                                            switch (day) {
                                                                case 0:
                                                                    return "Chủ Nhật";
                                                        ...
                                                        .join(", ");
                                                    const timeRange = `${schedule.times.departureTime}`;
